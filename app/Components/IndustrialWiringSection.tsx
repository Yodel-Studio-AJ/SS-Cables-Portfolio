...
interface IndustrialWiringSectionProps {
  title: string;
  ctaTopText: string;
...
/*animations*/
const slideVariants = (x: number, delay: number): Variants => ({
  hidden: { opacity: 0, x },
  visible: {
    opacity: 1,
    x: 0,
...
/* Reusable CTA component used for both Top and Bottom actions
*/
const CtaLink = ({
  text,
  href = "#",
...
export default function IndustrialWiringSection({
  title,
  ctaTopText,
  ctaBottomText,
  imageUrls,
}: IndustrialWiringSectionProps) {
  const sectionRef = useRef<HTMLElement>(null);
  const isInView = useInView(sectionRef, { once: true, amount: 0.35 });

  const sectionId = "industrial-wiring-section";
  const titleId = "industrial-wiring-title";
  ...
  const displayText = title.endsWith('.') ? title.slice(0, -1) : title;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <CtaLink
          text={ctaTopText}
          variants={slideVariants(-60, 0.1)}
          className="flex justify-end mb-8 md:mb-12"
        />

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-y-12 lg:gap-x-16 lg:gap-y-0 items-stretch">

          {/* Content Block */}
          <motion.div
            variants={slideVariants(60, 0.3)}
          ...
          {/* Image Block */}
          <motion.div
            variants={slideVariants(-60, 0.5)}
            className="order-2 lg:order-none lg:col-start-2 lg:row-start-1 lg:row-span-2 relative aspect-[4/3] lg:aspect-auto lg:h-full min-h-[300px] lg:min-h-[500px] rounded-3xl overflow-hidden shadow-sm bg-gray-100"
          >
            <AnimatePresence>
              <motion.div
                key={currentIndex}
          ...
          <CtaLink
            text={ctaBottomText}
            variants={slideVariants(60, 0.7)}
            className="order-3 lg:order-none lg:col-start-1 lg:row-start-2 lg:self-end lg:pb-4"
          />
